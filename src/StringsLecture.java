
import java.util.Scanner;

public class StringsLecture {
    public static void main(String[] args) {
        /* Strings are notated with double quotes in Java, Strings are a reference type, single quotes are for characters because they are a primitive type
        * The double equals will not work on two strings because strings are objects..so we have to a method
        * .equals() - Strings are immutable in java.  (means once a string is created it cannot be changed)
        * We never change a string, we change the object that tht string references
        * Think channel changer : if you're watching channel 12 and you change to 25 channel 12 doesn't go away, you're just shifting the focus(reference).
        * Concatination works in Java like JS
        *
        * to ignore capitalization - use .equalsIgnoreCase()
        *
        *
        * */

//        Uncomment the Scanner and String variables.

        Scanner sc = new Scanner(System.in).useDelimiter("\n");
        System.out.println("Please enter your name:");
//
        String input = sc.nextLine();

        String name = "Jane Doe";

        /*
         *  TO DO: Write the control statements below to check the input
         *  String using Java methods.
         */

        //  If the input is an exact match to String name...
        //  Print message: "Welcome Jane Doe!"
//        if(input.equals(name)) {
//            System.out.println("Welcome Jane Doe!");
//        }
//
//        //  If the input is not capitalized correctly...
//        //  Print message: "Names are proper nouns."
//
//        if(input.equalsIgnoreCase(name)){
//            System.out.println("Names are proper nouns...");
//        }
//
//        //  If the input starts with "Jane"...
//        //  Print message: "Hi Jane!"
//
//        if(input.startsWith("Jane")){
//            System.out.println("Hi, Jane!");
//        }
//
//        //  If the input ends with "Doe"...
//        //  Print message: "Doe, a deer..."
//        if(input.endsWith("Doe")){
//            System.out.println("Doe, a deer...");
//        }
        /*
         *  TO DO TOGETHER: Uncomment each line and view the results of each
         *  String manipulation method.
         */

//        System.out.println(name.charAt(3));
//        System.out.println(name.indexOf("Doe"));
//        System.out.println(name.lastIndexOf("e"));
//        System.out.println(name.length());
//        System.out.println(name.replace("Doe", "Smith"));
//        System.out.println(name.substring(3));
//          String nameSubString = name.substring(3);
//          System.out.println(nameSubString);
//        System.out.println(name.substring(0,3));
//        cutting point is where the index falls (think replace char at_ with wall)

//        System.out.println(name.toLowerCase());
//        System.out.println(name.toUpperCase());

        // trim is really useful for catching user errors on input.
//        String trimThis = "    too many spaces   ";
      //  if(trimThis.equals("too many spaces)")) // This wont work
//        System.out.println(trimThis);
//        System.out.println(trimThis.trim());
//        System.out.println(trimThis); //Same as the original
//
//        String trimmedVersion = trimThis.trim();
//        System.out.println(trimmedVersion);

        // How would we fix this?
//        System.out.println("Here is a backslash \\");

//        If we know we're going to take in a bunch of inputs and we know we're going to trim it, we could create a method that trims it at the beginning...

    }
}


